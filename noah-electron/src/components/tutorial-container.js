...
const TutorialContainer = ({ category }) => {
    ...
    return (
        ...
        <div key={category} className="mt-32 flex justify-center bg-transparent w-full h-full">
            {chosenTutorials.length > 0 ? (

                // If there are tutorials to display, map through them and display them in a grid of five columns.
                ...
                // If there are no tutorials to display, show a message so the screen isn't empty.
                <div className="flex justify-center items-center w-full h-full">
                    <span className="text-[#434141]">No tutorial content found!</span>
                </div>
            )}
        </div>
    );
